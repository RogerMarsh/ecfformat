...
# Copied from chessvalidate and modified to fit.
...
# Remove additional '.core' steps which existed in chessvalidate version.
...
class Configuration:
        ...
        if len(_items) == 0:
            self.set_configuration_values_from_text(
                self.get_configuration_text_and_values_for_items_from_file(
                    {
                        default[0]: default[1]
                        for default in self._DEFAULT_RECENTS
                    }
                )
            )
    ...
    def get_configuration_file_name(self):
        ...
        return self._RESULTS_CONF

    def get_configuration_file_path(self):
        """Return configuration file path."""
        return os.path.expanduser(os.path.join("~", self._RESULTS_CONF))
    ...
    def get_configuration_text_for_items_from_file(self, items, values=False):
        ...
        try:
            with open(
                os.path.expanduser(os.path.join("~", self._RESULTS_CONF))
            ) as config_file:
    ...
    def set_configuration_values_from_text(self, text, config_items=None):
        ...
        if config_items is None:
            config_items = {}
        default_values = {
            default[0]: default[1] for default in self._DEFAULT_RECENTS
        }
    ...
    def _save_configuration(self):
        """Save the configuration in file named in self._RESULTS_CONF."""
        config_text = []
        ...
        try:
            with open(
                os.path.expanduser(os.path.join("~", self._RESULTS_CONF)), "w"
            ) as config_file:
